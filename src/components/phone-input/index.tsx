...
import { useStateHistory } from "@/shared/hooks/useStateHistory";
  ...
  return (
    <div className={cn("flex gap-2", className)}>
      <Popover open={openCommand} onOpenChange={setOpenCommand} modal={true}>
        <PopoverTrigger asChild>
          ...
          >
            {selectedCountry?.name ? (
              <span className="relative top-[-6x] text-base">{selectedCountry.emoji}</span>
            ) : (
              "Selecione o pais"
            )}
        ...
        <PopoverContent className="p-0 w-max h-[300px]" align="start">
          <Command>
